...
function open_coverage(pkg)
    htmlfile = joinpath(dirname(pathof(pkg)), COVDIR, "index.html")
    if !isfile(htmlfile)
...
function clean_coverage(pkg)
    Coverage.clean_folder(dirname(pathof(pkg)))
    rm(joinpath(dirname(pkg, COVDIR)); force = true, recursive = true)
end
