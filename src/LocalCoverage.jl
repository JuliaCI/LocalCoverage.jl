...
Open the HTML coverage results for `pkg` if they exist.

See [`generate_coverage`](@ref).
"""
function open_coverage(pkg)
    htmlfile = Pkg.dir(COVDIR, "index.html")
    if !isfile(htmlfile)
        warn("Run generate_coverage(\"$(pkg)\") first.")
        return nothing
    end
    try
        if is_apple()
