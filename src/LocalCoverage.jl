...
- `json_summary = false` determines whether a JSON summary of coverage is generated.

- `json_summary_filename = "coverage-summary.json"` specifies the filename of the generated JSON summary.

- `json_comparison_summary_filename = nothing` specifies the path/string of previous coverage JSON summary to compare against.

- `json_summary_comparison_fail_on_decrease = false` determines whether to fail (throw an error) if the coverage has decreased.
...
function generate_coverage(pkg = nothing;
                           ...
                           file_list = [],
                           json_summary = false,
                           json_summary_filename = "coverage-summary.json",
                           json_comparison_summary_filename = nothing,
                           json_summary_comparison_fail_on_decrease = false)::PackageCoverage
    ...
    coverage = process_coverage(pkg; folder_list, file_list)
    if json_summary || !isnothing(json_comparison_summary_filename)
        out_path = generate_json_summary(coverage, json_summary_filename; test_args = test_args)
        if !isnothing(json_comparison_summary_filename)
            passed = compare_coverage_json_summaries(json_comparison_summary_filename, out_path, stdout)
            if !passed && json_summary_comparison_fail_on_decrease
