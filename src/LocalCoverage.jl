...
export generate_coverage, process_coverage, clean_coverage, report_coverage_and_exit,
    ...
    compare_coverage_json_summaries, JSONSummaryOptions
...
"""
JSON coverage summary configuration options.

$(FIELDS)
"""
Base.@kwdef struct JSONSummaryOptions
    "Whether to write the coverage JSON summary to disk"
    write::Bool = true
    "Filename of the generated JSON summary"
    filename::String = "coverage-summary.json"
    "Path/string of previous coverage JSON summary to compare against"
    comparison_filename::Union{Nothing,String} = nothing
    "Whether to fail (throw an error) if the coverage has decreased"
    comparison_fail_on_decrease::Bool = false
end
...
- `json_summary::Union{Nothing,JSONSummaryOptions} = nothing` determines whether and how a JSON summary of coverage is generated and compared. See [`JSONSummaryOptions`](@ref).
...
function generate_coverage(pkg = nothing;
                           ...
                           folder_list = ["src"],
                           file_list = [],
                           json_summary::Union{Nothing,JSONSummaryOptions} = nothing)::PackageCoverage
    ...
    coverage = process_coverage(pkg; folder_list, file_list)
    if !isnothing(json_summary)
        if json_summary.write || !isnothing(json_summary.comparison_filename)
            out_path = generate_json_summary(coverage, json_summary.filename; test_args = test_args)
            if !isnothing(json_summary.comparison_filename)
                passed = compare_coverage_json_summaries(json_summary.comparison_filename, out_path, stdout)
                if !passed && json_summary.comparison_fail_on_decrease
...
function compare_coverage_json_summaries(old::AbstractString, new::AbstractString, io::IO = stdout)
    ...
    if isempty(strip(old)) || (!is_json(old) && !isfile(old))
